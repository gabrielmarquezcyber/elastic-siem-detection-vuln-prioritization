...
# Output directory (consistent with your repo)
OUT_DIR = Path(r"C:\gm-interview-capstone\artifacts\figures")
...
# =========================================================
def safe_get_json(url, *, params=None, headers=None, timeout=120, max_retries=6, min_sleep=5, max_sleep=20):
    """
    Robust GET→JSON with retries, jitter, and helpful logs.
    Returns (ok, json_dict or None, last_status_code, last_text_snippet).
    """
    last_status = None
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if r.status_code in (429, 403, 502, 503, 504):
                ...
                print(f"[NVD] HTTP {r.status_code} (attempt {attempt}/{max_retries}) — retrying in {sleep_s}s")
                time.sleep(sleep_s)
                continue
            break
        except requests.RequestException as e:
            sleep_s = random.randint(min_sleep, max_sleep)
            print(f"[NVD] Request exception {e} (attempt {attempt}/{max_retries}) — retrying in {sleep_s}s")
            time.sleep(sleep_s)
...
kev_set = {item.get("cveID") for item in kev.get("vulnerabilities", []) if item.get("cveID")}
...
start_date = (datetime.utcnow() - timedelta(days=LOOKBACK_DAYS)).strftime("%Y-%m-%dT%H:%M:%S.000")
headers = {"User-Agent": "gm-capstone/1.0", "apiKey": NVD_API_KEY}
...
while True:
    params = {"lastModStartDate": start_date, "lastModEndDate": end_date, "resultsPerPage": PAGE_SIZE, "startIndex": start_index}
    ok, data, status, snippet = safe_get_json(nvd_base, params=params, headers=headers, timeout=120)
    ...
    for v in vulns:
        ...
        rows.append({"CVE": cve_id, "CVSS": float(cvss) if cvss is not None else 0.0})
...
colors = ['#d62728' if row["KEV"] == 1 else '#1f77b4' for _, row in chart_data_sorted.iterrows()]
bars = ax.barh(chart_data_sorted["CVE"], chart_data_sorted["Priority"], color=colors)
ax.set_xlabel("Priority Score", fontsize=12)
ax.set_title(f"CVE Priority: Top KEV vs Non-KEV — Last {LOOKBACK_DAYS}d", fontsize=14)
